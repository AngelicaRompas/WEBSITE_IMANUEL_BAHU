<?php
// keuangan_sampul.php - Laporan Detail Penerimaan Sampul Publik dengan Filter Pekan
    ...
    <div class="table-responsive rounded-3 border border-light-subtle shadow-sm">
        ...
        <?php if($sub_sampul === 'all'): ?>
            ...
            <table class="table table-hover align-middle mb-0 text-center text-nowrap" style="min-width: 600px; font-size: 0.9rem;">
                <thead class="bg-light text-secondary text-uppercase fw-bold" style="font-size: 0.75rem; letter-spacing: 0.5px;">
                    <tr>
                        <th class="py-3 text-start ps-4" style="width: 20%;">Kolom</th>
                        <th class="py-3 text-start ps-4" style="width: 60%;">Keterangan Jenis Sampul</th>
                ...
                <tbody>
                    ...
                    if(!empty($tanggal_minggu_list)):
                        $grand_total_all = 0;
                        // Ambil target tanggal satu hari minggu saja sesuai filter dropdown di atas
                        $tanggal_target = $tanggal_minggu_list[$minggu_pilih - 1];

                        $q_all = mysqli_query($koneksi, "SELECT kolom, kategori, nominal FROM keuangan_sampul_massal WHERE tanggal = '$tanggal_target' ORDER BY kolom ASC");
                    ...
                    <tr class="border-bottom border-light-subtle">
                        <td class="text-start ps-4 py-3 font-monospace text-secondary">Kolom <?= $r_all['kolom']; ?></td>
                        <td class="text-start ps-4 text-dark fw-medium"><?= $kategori_map[$r_all['kategori']] ?? $r_all['kategori']; ?></td>
                    ...
                    ?>
                    <tr>
                        <td colspan="3" class="py-5 text-muted text-center small">
                    ...
                    ?>
                    <!-- TOTAL BARIS FOOTER PEKANAN -->
                    <tr class="fw-bold bg-light" style="background: #f8fafc;">
                        <td colspan="2" class="text-start ps-4 py-3 text-secondary fw-bold" style="font-size: 0.75rem;">TOTAL PENGUMPULAN PEKANAN (MINGGU KE-<?= $angka_romawi[$minggu_pilih]; ?>)</td>
                    ...
                    <?php else: ?>
                    <tr><td colspan="3" class="py-5 text-muted">Gagal memetakan penanggalan bulan ini.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <?php else: ?>
            ...
            <table class="table table-hover align-middle mb-0 text-center text-nowrap" style="min-width: 700px; font-size: 0.9rem;">
                ...
                <tbody>
                    <?php 
                    // FIXED: Menggunakan SELECT * agar aman dari salah penamaan kolom keterangan/nama_keluarga
                    ...
                    if(mysqli_num_rows($q_spesifik) > 0):
                        ...
                        while($r_spes = mysqli_fetch_assoc($q_spesifik)):
                            ...
                            // Logika adaptif mendeteksi nama field keterangan pembawa yang tersedia di database Anda
                            $keterangan_tampil = $r_spes['keterangan'] ?? ($r_spes['nama'] ?? ($r_spes['nama_keluarga'] ?? '-'));
                    ?>
                    <tr class="border-bottom border-light-subtle">
                        ...
                        <td class="font-monospace text-secondary">Kolom <?= $r_spes['kolom']; ?></td>
