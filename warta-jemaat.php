...
<head>
    ...
    <link rel="stylesheet" href="assets/css/style_warta.css">
...
<div class="container py-5 mt-5">
    <?php if (!$akses_warta): ?>
        <!-- Tampilan Halaman Terkunci (Input Kode Referral) -->
        <div class="row justify-content-center">
            <div class="col-md-4 glass-card p-5 text-center mt-5">
                <i class="bi bi-shield-lock text-purple fs-1 mb-3 d-block"></i>
                <h5 class="fw-bold my-3">Area Terbatas</h5>
                <p class="text-muted small mb-4">Silakan masukkan kode akses untuk melihat dokumen warta jemaat.</p>
                <form method="POST">
                    <input type="text" name="kode_referral" class="form-control mb-3 text-center rounded-pill py-2" placeholder="Masukkan Kode..." required>
    ...
    <?php else: ?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 mt-3">
            <h3 class="fw-bold"><i class="bi bi-collection-play me-2"></i> <?php echo $detail_tgl ? 'Detail Warta Digital' : 'Arsip Warta Jemaat'; ?></h3>
            <a href="warta-jemaat.php?logout=1" class="btn btn-outline-danger btn-sm rounded-pill px-4 fw-bold">Tutup Akses</a>
        </div>

        <?php if ($detail_tgl): ?>
            <!-- Tampilan Mode Detail (Petugas per Sesi) -->
            <a href="warta-jemaat.php" class="text-decoration-none mb-4 d-inline-block fw-bold"><i class="bi bi-arrow-left me-1"></i> Kembali ke Daftar Tanggal</a>
            <div class="row g-4">
                ...
                <div class="col-md-4">
                    <div class="glass-card p-4 h-100 text-center position-relative">
                        ...
                        <?php if(!empty($row['foto_khadim'])): ?>
                            <img src="admin/assets/images-khadim/<?php echo htmlspecialchars($row['foto_khadim']); ?>" class="khadim-img mb-3" alt="Foto Khadim <?php echo htmlspecialchars($row['nama_khadim']); ?>">
                        <?php else: ?>
                            <div class="khadim-img d-flex align-items-center justify-content-center bg-light mx-auto mb-3">
                        ...
                        <?php if(!empty($row['file_pdf'])): ?>
                            <div class="mt-4 pt-3 border-top" style="border-color: rgba(0,0,0,0.05) !important;">
                                <a href="admin/assets/document_warta/<?php echo htmlspecialchars($row['file_pdf']); ?>" target="_blank" class="btn btn-sm btn-purple w-100 py-2 fw-bold">
                                    <i class="bi bi-cloud-arrow-down-fill me-2"></i> Unduh PDF Warta Lengkap
                                </a>
                            </div>
                        <?php endif; ?>
        ...
        <?php else: ?>
            <!-- Tampilan Mode Daftar Tanggal (Awal Masuk) -->
                ...
                <?php if(mysqli_num_rows($query) > 0): ?>
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item d-flex justify-content-between align-items-center p-3 mb-2 border-0 rounded" style="background-color: transparent;">
                            ...
                            <a href="warta-jemaat.php?detail=<?php echo $row['tanggal']; ?>" class="btn btn-purple rounded-pill px-4 fw-bold shadow-sm">Buka Digital</a>
                        </div>
                        <?php endwhile; ?>
                    </div>
                <?php else: ?>
